...
def cam_to_ego(points, rot, trans, intrins):
    ...
    to the ego frame
    """
    points = paddle.concat((points[:2] * points[2:3], points[2:3]))
...
def img_transform(img, post_rot, post_tran,
    ...
    post_rot *= resize
    post_tran -= paddle.Tensor(np.array(crop[:2]))
    if flip:
        A = paddle.Tensor(np.array([[-1., 0.], [0., 1.]], dtype="float32"))
        b = paddle.Tensor(np.array([crop[2] - crop[0], 0.]))
        # print(A.dtype,post_rot.dtype)
        post_rot = A.matmul(post_rot)
        post_tran = A.matmul(post_tran) + b
    A = get_rot(rotate / 180 * np.pi)
    b = paddle.Tensor(np.array([crop[2] - crop[0], crop[3] - crop[1]])) / 2
    b = A.matmul(-b) + b
    post_rot = A.matmul(post_rot)
    post_tran = A.matmul(post_tran) + b
...
def gen_dx_bx(xbound, ybound, zbound):
    dx = paddle.Tensor(np.array([row[2] for row in [xbound, ybound, zbound]]))
    bx = paddle.Tensor(np.array([row[0] + row[2] / 2.0 for row in [xbound, ybound, zbound]]))
    nx = paddle.Tensor(np.array([(row[1] - row[0]) / row[2] for row in [xbound, ybound, zbound]], dtype="int64"))
...
def cumsum_trick(x, geom_feats, ranks):
    x = x.cumsum(0)
    kept = paddle.ones([x.shape[0]], dtype=paddle.bool)
...
class SimpleLoss(paddle.nn.Layer):
    def __init__(self, pos_weight):
        super(SimpleLoss, self).__init__()
        self.loss_fn = paddle.nn.BCEWithLogitsLoss(pos_weight=paddle.to_tensor(pos_weight))
...
def get_batch_iou(preds, binimgs):
    ...
    with paddle.no_grad():
        pred = (preds > 0)
        tgt = paddle.cast(binimgs, 'bool')
        intersect = (pred & tgt).sum().item()
